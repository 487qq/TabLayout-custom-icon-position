...
import com.max.library.view.v7.AppCompatDrawableManager;
...
public class TabLayout extends HorizontalScrollView {
    ...
    public void addTab(@NonNull Tab tab,float weight) {
        addTab(tab, mTabs.isEmpty(),weight);
    }
    ...
    public void addTab(@NonNull Tab tab, boolean setSelected,float weight) {
        if (tab.mParent != this) {
            throw new IllegalArgumentException("Tab belongs to a different TabLayout.");
        }

        addTabView(tab, setSelected,weight);
        configureTab(tab, mTabs.size());
        if (setSelected) {
            tab.select();
        }
    ...
    private void addTabView(Tab tab, boolean setSelected,float weight) {
        final TabView tabView = createTabView(tab);
        mTabStrip.addView(tabView, createLayoutParamsForTabs(weight));
        if (setSelected) {
            tabView.setSelected(true);
        }
    }
    ...
    private LinearLayout.LayoutParams createLayoutParamsForTabs(float weight) {
        final LinearLayout.LayoutParams lp = new LinearLayout.LayoutParams(
                LayoutParams.WRAP_CONTENT, LayoutParams.MATCH_PARENT);
        updateTabViewLayoutParams(lp,weight);
        return lp;
    }
    ...
    private void updateTabViewLayoutParams(LinearLayout.LayoutParams lp,float weight) {
        if (mMode == MODE_FIXED && mTabGravity == GRAVITY_FILL) {
            lp.width = 0;
            lp.weight = weight;
        } else {
    ...
    public TabView getTabViewAt(int position){
        return (TabView) mTabStrip.getChildAt(position);
    }
    ...
    public static final class Tab {
        ...
        @NonNull
        public Tab setIcon(@Nullable Drawable icon,int type) {
            mType = type;
            mIcon = icon;
            if (mPosition >= 0) {
                mParent.updateTab(mPosition);
        ...
        public Tab setIcon(@DrawableRes int resId,int type) {
            return setIcon(AppCompatDrawableManager.get().getDrawable(mParent.getContext(), resId),type);
        }
                        ...
                        case LEFT:
                            ...
                            if (mIconView == null) {
                                ImageView iconView = (ImageView) LayoutInflater.from(getContext())
                                        .inflate(com.max.library.R.layout.design_layout_tab_icon, this, false);
                                addView(iconView,0);
                                mIconView = iconView;
                            }
                            if (mTextView == null) {
                                TextView textView = (TextView) LayoutInflater.from(getContext())
                                        .inflate(com.max.library.R.layout.design_layout_tab_text, this, false);
                                addView(textView);
                                mTextView = textView;
...
//                                textView.setBackgroundColor(Color.parseColor("#FF4081"));
                            ...
                            if (mIconView == null) {
                                ImageView iconView = (ImageView) LayoutInflater.from(getContext())
                                        .inflate(com.max.library.R.layout.design_layout_tab_icon, this, false);
//                                iconView.setBackgroundColor(Color.parseColor("#FF4081"));
                                addView(iconView);
                                mIconView = iconView;
                            }
                            break;
            ...
            if (iconView != null) {
                ...
                if (hasText && iconView.getVisibility() == VISIBLE) {
                    ...
                    if (mTab.getType()!=-1){
                        switch (mTab.getType()) {
                            case LEFT:
                                rightMargin = dpToPx(DEFAULT_GAP_TEXT_ICON);
                                break;
                            case RIGHT:
                                leftMargin = dpToPx(DEFAULT_GAP_TEXT_ICON);
                                break;
                            case TOP:
                                bottomMargin = dpToPx(DEFAULT_GAP_TEXT_ICON);
                                break;
                            case BOTTOM:
                                topMargin = dpToPx(DEFAULT_GAP_TEXT_ICON);
                                break;
                        }
                    }
                }
                if (bottomMargin != lp.bottomMargin || topMargin != lp.topMargin||leftMargin!=lp.leftMargin||rightMargin!=lp.rightMargin) {
    ...
    private class SlidingTabStrip extends LinearLayout {
        ...
        private int mIndicatorPaddingLeft=15;
        private int mIndicatorPaddingRight=15;
        ...
        private void setIndicatorPosition(int left, int right) {
            if (left != mIndicatorLeft || right != mIndicatorRight) {
                ...
                mIndicatorLeft = left+mIndicatorPaddingLeft;
                mIndicatorRight = right-mIndicatorPaddingRight;
                ViewCompat.postInvalidateOnAnimation(this);
            }
        }
